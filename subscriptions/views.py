...
def paypal_webhook_view(request):
    ...
    try:
        ...
        #logger.error("Webhook Error: Invalid JSON in request body.")
    ...
    if not all([transmission_id, transmission_time, cert_url, auth_algo, transmission_sig, webhook_id_from_settings]):
        #logger.error("Webhook Error: Missing required PayPal headers or webhook ID configuration.")
    ...
    if not access_token:
        #logger.error("Webhook Error: Could not obtain PayPal access token for verification.")
    ...
    try:
        ...
        if verification_result.get("verification_status") != "SUCCESS":
            #logger.warning(f"Webhook verification failed: {verification_result}")
            return JsonResponse({'error': 'Invalid webhook signature', 'details': verification_result}, status=403) # Forbidden

    except requests.exceptions.HTTPError as e:
        #logger.error(f"HTTPError during PayPal API call for webhook verification: {e}")
        ...
        #logger.error(f"RequestException during PayPal API call for webhook verification: {e}")
        return JsonResponse({'error': f'Network error during PayPal API communication: {e}'}, status=503) # Service Unavailable
    except Exception as e:
        #logger.exception("Unexpected error during webhook verification process:") # Logs full traceback
    ...
    try:
        if event_type == "BILLING.SUBSCRIPTION.CANCELLED":
            ...
            if subscription_id:
                ...
                if created:
                    pass
                     #logger.warning(f"Subscription {subscription_id} was newly created by ACTIVATED webhook. Status set to ACTIVE. User association might be needed.")
                else:
                    pass
                    #logger.info(f"Subscription {subscription_id} status updated to ACTIVE.")
            else:
                pass
                #logger.warning(f"Event {event_type} missing resource.id.")
            ...
            if subscription_id:
                ...
                #logger.info(f"Subscription {subscription_id} status updated to SUSPENDED.")
            else:
                pass
                #logger.warning(f"Event {event_type} missing resource.id.")
            ...
            if subscription_id:
                ...
                #logger.info(f"Payment completed for subscription {subscription_id}. Status ensured/updated to ACTIVE.")
                # You might also want to update a `last_payment_date` or `next_billing_date` if you track those
            else:
                pass
                #logger.warning(f"Event {event_type} missing resource.billing_agreement_id.")
        ...
        #logger.exception(f"Error processing webhook event {event_type}:") # Logs full traceback
